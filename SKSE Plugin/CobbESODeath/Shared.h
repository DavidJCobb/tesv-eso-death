...
namespace cobb {
   ...
   namespace skyrim {
      void dispose(void*, bool); // Calls the Dispose method on a target object, via ASM.
      ...
      //template<typename T> T* heap_create_struct(); // Create an object on Skyrim's heap. // usage causes linker errors
   };
};
